...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        Deque<Integer> baekjoon.stack = new ArrayDeque<>();

        // ② push — 원소 추가 (스택 = addFirst / push)
        baekjoon.stack.push(10);   // [10]
        baekjoon.stack.push(20);   // [20, 10]  ← 앞이 top

        // ③ peek — top 원소 조회 (제거하지 않음)
        int top = baekjoon.stack.peek();  // 20

        // ④ pop — top 원소 제거 및 반환
        int val = baekjoon.stack.pop();   // 20, baekjoon.stack = [10]

        // ⑤ isEmpty 체크 — pop 전에 반드시 확인
        if (!baekjoon.stack.isEmpty()) {
            baekjoon.stack.pop();
        }

        // ⑥ size 확인
        int size = baekjoon.stack.size();
    }
}
```
...
```java
// 핵심 로직만
Deque<Character> baekjoon.stack = new ArrayDeque<>();
for (char c : s.toCharArray()) {
    if (c == '(' || c == '[' || c == '{') {
        baekjoon.stack.push(c);                        // 열린 괄호 → push
    } else {
        if (baekjoon.stack.isEmpty()) return false;    // 닫힌 괄호인데 스택 빈 경우 → 불균형
        char top = baekjoon.stack.pop();
        if (!isMatch(top, c)) return false;   // 매칭 실패 시 불균형
    }
}
return baekjoon.stack.isEmpty();                       // 남은 열린 괄호가 없어야 균형
```
...
Deque<Integer> baekjoon.stack = new ArrayDeque<>();   // 인덱스 저장
int[] result = new int[arr.length];
Arrays.fill(result, -1);                     // 기본값 -1 (작은 값 없음)

for (int i = 0; i < arr.length; i++) {
    // 현재 값이 스택 top의 값보다 작으면 → top의 "오른쪽 첫 작은 값" = 현재
    while (!baekjoon.stack.isEmpty() && arr[baekjoon.stack.peek()] > arr[i]) {
        result[baekjoon.stack.pop()] = arr[i];        // pop된 인덱스의 답 확정
    }
    baekjoon.stack.push(i);                           // 현재 인덱스를 스택에 추가
}
```
...
코딩테스트에서도 TLE 원인이 될 수 있으므로 반드시 `Deque<Integer> baekjoon.stack = new ArrayDeque<>()` 사용.
...
반드시 인덱스를 push하고, 값은 arr[baekjoon.stack.peek()]로 접근.

**④ 괄호 검증 마지막 체크 실수**
루프 끝에서 baekjoon.stack.isEmpty() 최종 확인을 빠뜨리면 "(((" 같은 열린 괄호만 남은 케이스를 통과시킬 수 있다.
